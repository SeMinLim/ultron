...
int main(void)
{
    ...
    if (cuckoo_hash_init(&ch, 8) != 0) {
        fprintf(stderr, "cuckoo_hash: init failed\n");
        return 1;
    }

    if (cuckoo_hash_insert(&ch, 1, 100) != 0) {
        fprintf(stderr, "cuckoo_hash: insert failed\n");
        return 1;
    }
    if (cuckoo_hash_lookup(&ch, 1, &val) != 1 || val != 100) {
        fprintf(stderr, "cuckoo_hash: lookup failed\n");
        return 1;
    }

    if (cuckoo_hash_insert(&ch, 1, 200) != 0) {
        fprintf(stderr, "cuckoo_hash: update failed\n");
        return 1;
    }
    if (cuckoo_hash_lookup(&ch, 1, &val) != 1 || val != 200) {
        fprintf(stderr, "cuckoo_hash: update lookup failed\n");
        return 1;
    }

    if (cuckoo_hash_remove(&ch, 1) != 1) {
        fprintf(stderr, "cuckoo_hash: remove failed\n");
        return 1;
    }
    if (cuckoo_hash_lookup(&ch, 1, &val) != 0) {
        fprintf(stderr, "cuckoo_hash: lookup after remove failed\n");
        return 1;
    }

    for (uint64_t i = 0; i < 10; i++) {
        if (cuckoo_hash_insert(&ch, i, i * 10) != 0) {
            fprintf(stderr, "cuckoo_hash: bulk insert failed at %lu\n", (unsigned long)i);
            return 1;
        }
    }
    for (uint64_t i = 0; i < 10; i++) {
        if (cuckoo_hash_lookup(&ch, i, &val) != 1 || val != i * 10) {
            fprintf(stderr, "cuckoo_hash: bulk lookup failed at %lu\n", (unsigned long)i);
            return 1;
        }
    }

    cuckoo_hash_free(&ch);
    printf("cuckoo_hash: all tests passed\n");
    return 0;
}
#endif
